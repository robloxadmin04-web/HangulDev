  ...
  items.forEach(item => {
    ...
    row.innerHTML =
      ...
      '<div class="vocab-example">' +
        ...
        '<div>' + item.ex_en + '</div>' +
      '</div>' +
      '<button class="quick-note-btn" type="button" data-note-kr="' + item.kr + '" data-note-en="' + item.en + '" data-note-section="vocabulary">＋ Note</button>';
...
function renderGrammar() {
  ...
  GRAMMAR_LESSONS.forEach(lesson => {
    ...
    el.innerHTML =
      ...
      '<div class="grammar-lesson-body">' +
        ...
        '<button class="quick-note-btn" type="button" data-note-kr="' + lesson.sentence + '" data-note-en="' + lesson.title + '" data-note-section="grammar">＋ Note</button>' +
      '</div>';
  ...
  if (typeof bindQuickNoteButtons === 'function') bindQuickNoteButtons(container);
...
function openNoteEditor(noteId, sourceContext) {
  ...
  if (existing) {
    ...
    if (existing.source && existing.source.kr) {
      ...
      sourceBox.textContent = existing.source.kr + (existing.source.en ? ' — ' + existing.source.en : '');
    } else {
  ...
  if (existing) backdrop.dataset.pendingSource = existing.source ? JSON.stringify(existing.source) : '';
...
function saveNoteFromEditor() {
  ...
  if (!title && !body.trim()) { closeNoteEditor(); return; }

  if (notesEditingId) {
    NotesStorage.update(notesEditingId, { title: title || 'Untitled note', tag: fieldTag.value, body });
  } else {
    let source = null;
    try { source = backdrop.dataset.pendingSource ? JSON.parse(backdrop.dataset.pendingSource) : null; } catch (e) {}
    NotesStorage.add({ title: title || 'Untitled note', tag: fieldTag.value, body, source });
  }
  closeNoteEditor();
  renderNotes();
}
...
function renderNotes() {
  ...
  if (tagFilterEl) {
    ...
    tagFilterEl.innerHTML = '<option value="">All tags</option>' +
      tags.map(t => '<option value="' + t + '">' + t.charAt(0).toUpperCase() + t.slice(1) + '</option>').join('');
  ...
  notes.forEach(n => {
    ...
    card.innerHTML =
      '<div class="note-card-head">' +
        '<div class="note-card-title">' + escapeNoteHtml(n.title) + '</div>' +
        '<div class="note-card-tag">' + escapeNoteHtml(n.tag || 'general') + '</div>' +
      '</div>' +
      (n.source && n.source.kr ? '<div class="note-card-source">' + escapeNoteHtml(n.source.kr) + '</div>' : '') +
      '<div class="note-card-body">' + escapeNoteHtml(n.body || '') + '</div>' +
      '<div class="note-card-foot">' +
        '<span class="note-card-date">' + formatNoteDate(n.updatedAt) + '</span>' +
    ...
    if (pdfBtn) pdfBtn.addEventListener('click', (e) => { e.stopPropagation(); exportNotesPdf([n], n.title); });
    if (shareBtn) shareBtn.addEventListener('click', (e) => { e.stopPropagation(); shareNotes([n]); });
...
function exportNotesPdf(notes, title) {
  if (typeof PracticePDFExport === 'undefined') { alert('PDF export is unavailable.'); return; }
  PracticePDFExport.exportNotes(notes, { title: title || 'My Korean Study Notes' });
}

function notesToPlainText(notes) {
  return notes.map(n => {
    const src = n.source && n.source.kr ? n.source.kr + (n.source.en ? ' — ' + n.source.en : '') + '\n' : '';
    return '• ' + n.title + ' [' + (n.tag || 'general') + ']\n' + src + (n.body || '') + '\n';
  }).join('\n---\n\n');
}

async function shareNotes(notes) {
  const text = notesToPlainText(notes);
  const shareTitle = notes.length === 1 ? notes[0].title : 'My Korean Study Notes (' + notes.length + ')';
  if (navigator.share) {
    try {
...
function initNotes() {
  ...
  if (newBtn) newBtn.addEventListener('click', () => openNoteEditor(null, null));
  ...
  if (backdrop) backdrop.addEventListener('click', (e) => { if (e.target === backdrop) closeNoteEditor(); });
  document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && backdrop && backdrop.classList.contains('open')) closeNoteEditor(); });
  ...
  if (exportAllBtn) exportAllBtn.addEventListener('click', () => {
    ...
    if (!all.length) { alert('You don\'t have any notes yet.'); return; }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const validSections = ['overview','hangul','reading','vocabulary','grammar','lessons','builder','practice','review','notes'];
